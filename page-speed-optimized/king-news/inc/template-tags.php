...
function king_news_footer_logo() {
	...
	$logo_id  = king_news_get_image_id_by_url( king_news_render_theme_url( $logo_url ) );
 ...
 * @return void
 */
function king_news_blog_content($length = 25) {
	...
	switch ( $blog_content ) {
		...
		case 'excerpt':
			king_news_post_excerpt( array( 'length' => $length, 'more' => '&hellip;' ) );
			break;
	}
...
function king_news_top_sign_register() {
	echo "<div class='top-panel__register'>";
		echo "<a href=' " . wp_login_url() . "' title='Login'>" . __( 'Login', 'king_news' ) ."</a>";
		wp_register('', '');
	echo "</div>";
